...
{#if !$Portfolio.fetching}
  <div
    ...
    style="background-image: url({$Portfolio.data?.allPortfolio[0].banner?.asset
      ?.url}); background-size: auto; "
  ></div>
  ...
  >
    <img
      ...
      src={$Portfolio.data?.allPortfolio[0].hero?.contentUrl}
      alt=""
    />
    <div class="grid grid-flow-row gap-y-6 md:col-span-12">
      <div class="flex flex-col gap-y-2 col-span-6">
        <h1 class="font-space-mono text-dh-orange font-bold text-3xl">
          {$Portfolio.data?.allPortfolio[0].title}
        </h1>
        <p class="font-space-mono text-dh-white">
          > {$Portfolio.data?.allPortfolio[0].details?.project}
        </p>
        <p class="font-space-mono text-dh-white">
          > {$Portfolio.data?.allPortfolio[0].details?.projectType}
        </p>
        <p class="font-space-mono text-dh-white">
          > {new Date(
      ...
      <img
        ...
        src={$Portfolio.data?.allPortfolio[0].media?.contentUrl}
  ...
  <div class="mt-12">
    ...
    <img
      ...
      src={$Portfolio.data?.allPortfolio[0].mockup?.contentUrl}
      alt="rare club poster"
    />
  </div>
{/if}
